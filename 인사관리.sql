...
is
CURSOR emp_cursor IS
    select employee_id, last_name,ceil(months_between(sysdate,hire_date)/12), to_char(sysdate,'yy')-to_char(hire_date,'yy') as workyear
    from employees
    ...
    v_workyear2 varchar2(100);
    e_dept_del_fail EXCEPTION;
BEGIN

    OPEN emp_cursor;
    LOOP
        FETCH emp_cursor INTO v_eid,v_ename,v_workyear,v_workyear2;
        EXIT WHEN emp_cursor%NOTFOUND;
        dbms_output.put_line(v_eid||' '||v_ename||' '||v_workyear||'년 차'||v_workyear2||'년 차'); 
    END LOOP;
    IF emp_cursor%ROWCOUNT = 0 then
        RAISE e_dept_del_fail;
    END IF;
...
select 
    ...
    round(mod(months_between(sysdate,hire_date),12))
from employees;
...
execute DBMS_OUTPUT.PUT_LINE(y_sum(10,5));

--내부에 DML in,up,del 가 존재하지 않음
--리턴에 사용된 데이터 타입이 sql이 사용하는 number,date,varchar2,char 중 하나
-- -> 두가지 조건을 만족하는 경우 해당 함수는 sql에서 사용가능
...
select emp.last_name, e.last_name
from employees e join employees emp on e.employee_id = emp.manager_id;
...
execute DBMS_OUTPUT.PUT_LINE(get_mgr(&사원번호));
...
begin
    ...
    return (sal_year+(sal_year*comi))*12;
end;    
/
